...
function imagesSearcher(text){
    ...
    let regexPng=/[A-Za-z]*.png\b/gi ;
    let resPng = text.match(regexPng);
    
    if(resPng!=null)
    for(let i=0;i<resPng.length;i++)
    arr.push(resPng[i]);


    let regexJpg=/[A-Za-z]*.jpg\b/gi ;
    let resJpg = text.match(regexJpg);

    if(resJpg!= null)
    for(let i=0;i<resJpg.length;i++)
    arr.push(resJpg[i]);

    let regexJpeg=/[A-Za-z]*.jpeg\b/gi ;
    let resJpeg = text.match(regexJpeg);

    if(resJpeg!=null )
    for(let i=0;i<resJpeg.length;i++)
    arr.push(resJpeg[i]);
